           ...
           <v-col cols="12" sm="3"  style="color:rgb(31, 29, 29);font-size:25px;font-weight: normal;">
            Pendientes: {{ cuentaCantidad}}
                ...
                <div  v-for="(cla,index) in clasesEstudiante" :key="index">
                 <div v-for="(claEs,i) in cla" :key="i" style="background-color:#D4D4D4;height:115px;padding:4px;border-radius:10px;margin:10px">
                   
                    
                  <div style="float:left">
                   <v-row>
                   <v-col cols="12" sm="6" style="float:left">
                    <h2>Tema: </h2>
                    <h4>{{claEs.name}}</h4>
                   </v-col></v-row>
                </div>
                <div style="height:80px;float:right; width: 70px; display: flex;justify-content: center;align-items: center;">
                  <button @click="irClase(i)"> <b-icon   style="color:green;" icon="book-half" font-scale="3"></b-icon></button>
                  </div>
                    <v-row style="float:right; margin-left:30px">
                       
                       <v-col cols="12" sm="12" style="float:left"> 
                           <h2>Descripción</h2>
                            <h4 >
                               {{claEs.descripcion}}
                            </h4></v-col></v-row>
